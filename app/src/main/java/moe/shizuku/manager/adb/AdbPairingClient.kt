...
class AdbPairingClient(private val host: String, private val port: Int, private val pairCode: String, private val key: AdbKey) : Closeable {
    ...
    /**
     * Reflective bridge to the platform-hidden
     * `com.android.org.conscrypt.Conscrypt.exportKeyingMaterial(SSLSocket, String, byte[], int)`.
     * The class is not on the compile classpath, and the call is blocked by the hidden-API
     * policy on API 28+, so [EmbeddedAdbInit.ensure] must have installed the "" exemption first.
     */
    private fun exportKeyingMaterial(socket: SSLSocket, label: String, context: ByteArray?, length: Int): ByteArray {
