    ...
    fun readPreferencesFromBackup(prefsDir: File): PreferencesBackupContents {
        ...
        prefsDir.listFiles { file -> file.extension == "json" }?.forEach { file ->
            try {
                ...
                if (name == brobata.physiboard.SettingsMigration.LEGACY_PREFS) {
                    // A backup taken before 2.0 renamed the file.
                    result[brobata.physiboard.SettingsMigration.PREFS] =
                        brobata.physiboard.SettingsMigration.translateRestoredEntries(entries)
                } else {
                    result[name] = entries
                }
            } catch (e: Exception) {
