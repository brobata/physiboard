...
object AutoCorrector {
    ...
    fun loadCorrections(assets: AssetManager, context: Context? = null) {
        try {
            ...
            if (context != null) {
                try {
                    val prefs = brobata.physiboard.SettingsManager.getPreferences(context)
                    ...
                    for ((key, value) in allPrefs) {
                        if (key.startsWith("auto_correct_custom_") && value is String) {
                            ...
                            if (languageCode !in standardLocales) {
                                try {
                                    val customCorrections = brobata.physiboard.SettingsManager.getCustomAutoCorrections(context, languageCode)
                                    if (customCorrections.isNotEmpty()) {
                                        val customJson = correctionsToJson(customCorrections)
    ...
    fun getCorrection(word: String, locale: String? = null, context: Context? = null): String? {
        ...
        if (context != null) {
            val enabledLanguages = brobata.physiboard.SettingsManager.getAutoCorrectEnabledLanguages(context)
    ...
    fun processText(
        ...
        val enabledLanguages = if (context != null) {
            brobata.physiboard.SettingsManager.getAutoCorrectEnabledLanguages(context)
        } else {
            emptySet<String>()
        }
        ...
        val hasHardBoundary = trailing.any { ch ->
            val normalized = brobata.physiboard.core.Punctuation.normalizeApostrophe(ch)
            !normalized.isWhitespace() && normalized !in brobata.physiboard.core.Punctuation.BOUNDARY
        }
        if (hasHardBoundary) {
            return null
        }
