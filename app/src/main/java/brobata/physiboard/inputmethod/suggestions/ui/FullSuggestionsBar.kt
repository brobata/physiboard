package brobata.physiboard.inputmethod.suggestions.ui
...
import brobata.physiboard.inputmethod.suggestions.SuggestionButtonHandler
import brobata.physiboard.inputmethod.VariationButtonHandler
import brobata.physiboard.inputmethod.ui.HamburgerMenuView
import brobata.physiboard.inputmethod.ui.KeyboardThemeColors
import brobata.physiboard.inputmethod.ui.ModifierIndicatorView
import brobata.physiboard.inputmethod.StatusBarController
import brobata.physiboard.inputmethod.statusbar.StatusBarButtonHost
import brobata.physiboard.inputmethod.statusbar.StatusBarButtonId
...
import brobata.physiboard.inputmethod.statusbar.StatusBarButtonPosition
...
class FullSuggestionsBar(
    ...
    private val targetHeightPx: Int
        get() = fixedHeightDp?.let { dpToPx(it.toFloat()) }
            ?: dpToPx(BASE_HEIGHT_DP * (themeOverride?.suggestionsHeightScale ?: 1f).coerceIn(brobata.physiboard.KEYBOARD_THEME_BAR_HEIGHT_MIN, brobata.physiboard.KEYBOARD_THEME_BAR_HEIGHT_MAX))
    ...
    private fun hasDictionaryForCurrentSubtype(): Boolean {
        return try {
            ...
            brobata.physiboard.core.suggestions.AndroidDictionaryRepository.hasDictionaryForLocale(context, langCode)
        } catch (e: Exception) {
            false
        }
    }
    ...
    private fun renderSlots(
        ...
        val heightScale = (themeOverride?.suggestionsHeightScale ?: 1f).coerceIn(brobata.physiboard.KEYBOARD_THEME_BAR_HEIGHT_MIN, brobata.physiboard.KEYBOARD_THEME_BAR_HEIGHT_MAX)
