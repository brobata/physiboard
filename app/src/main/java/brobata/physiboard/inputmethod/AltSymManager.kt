...
class AltSymManager(
    ...
    private fun autoSpacePunctuation(): String {
        return context?.let { SettingsManager.getAutoSpacePunctuation(it) }
            ?: brobata.physiboard.core.Punctuation.DEFAULT_AUTO_SPACE
    }
    ...
    fun reloadSymMappings() {
        if (context != null) {
            val customMappings = brobata.physiboard.SettingsManager.getSymMappings(context)
    ...
    fun reloadSymMappings2() {
        if (context != null) {
            val customMappings = brobata.physiboard.SettingsManager.getSymMappingsPage2(context)
            if (customMappings.isNotEmpty()) {
                symKeyMap2.clear()
    ...
    fun handleAltCombination(
        ...
        return if (altChar != null) {
            ...
            val frenchSpacedPunctuation = altChar.length == 1 &&
                ...
                brobata.physiboard.core.Punctuation.commitFrenchSpacedPunctuation(inputConnection, altChar[0])
                    ...
                    "sym", "sym_symbols", "sym_emoji" -> {
                        ...
                        if (!symChar.isNullOrEmpty()) {
                            ...
                            val frenchSpacedPunctuation = symChar.length == 1 &&
                                context?.let { SettingsManager.shouldApplyFrenchPunctuationSpacing(it) } == true &&
                                brobata.physiboard.core.Punctuation.commitFrenchSpacedPunctuation(inputConnection, symChar[0])
                            if (frenchSpacedPunctuation) {
