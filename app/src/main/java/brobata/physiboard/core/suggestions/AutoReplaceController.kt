...
class AutoReplaceController(
    ...
    private fun hasTrailingHardBoundary(textBeforeCursor: String): Boolean {
        ...
        while (i >= 0) {
            val normalized = brobata.physiboard.core.Punctuation.normalizeApostrophe(textBeforeCursor[i])
            if (normalized.isWhitespace() || normalized in brobata.physiboard.core.Punctuation.BOUNDARY) {
                i--
                continue
            }
