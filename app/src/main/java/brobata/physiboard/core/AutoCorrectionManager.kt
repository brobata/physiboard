package brobata.physiboard.core
...
class AutoCorrectionManager(
        ...
        val charsToDelete = if (lastChars.endsWith(correction.correctedWord)) {
            ...
            while (i >= 0 &&
                ...
                (textBeforeCursor[i].isWhitespace() ||
                        textBeforeCursor[i] in brobata.physiboard.core.Punctuation.BOUNDARY)
            ) {
                deleteCount++
                i--
            }
    ...
    fun handleBoundaryKey(
        ...
        val boundaryChar: Char? = boundaryCharOverride ?: when {
            isSpaceKey -> ' '
            isEnterKey -> '\n'
            punctuationChar != null && punctuationChar in brobata.physiboard.core.Punctuation.BOUNDARY -> punctuationChar
            else -> null
        }
        ...
        fun commitCommaSpaceIfEnabled(): Boolean {
            return commitBoundary &&
                brobata.physiboard.SettingsManager.getCommaSpace(context) &&
        ...
        val punctuationChars = brobata.physiboard.core.Punctuation.BOUNDARY
        ...
        if (isPunctuationBoundary && AutoSpaceTracker.isPending()) {
            ...
            if (boundaryChar in brobata.physiboard.SettingsManager.getAutoSpacePunctuation(context)) {
        ...
        if (commitBoundary) {
            ...
            when {
                ...
                isPunctuationBoundary -> {
                    if (
                        !brobata.physiboard.SettingsManager.shouldApplyFrenchPunctuationSpacing(context) ||
